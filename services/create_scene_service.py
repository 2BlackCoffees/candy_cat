...
from domain.all_scores import ScoreHandler
...
class CreateSceneService(WinLostManagement, GameTaskChanger):
    ...
    def __create_main_sprites(self) -> None:
        ...
        self.event_dispatcher.subscribe_start_stop(self)
    ...
    def inform_player_lost(self):
        ...
        if self.remaining_balls > 0:
            self.message = ["You beginner, you lost :-)",
                            "You have another %d ball(s)" % self.remaining_balls]
    ...
    def next_task(self):
        ...
        elif self.game_state in [ GameState.SHOWING_SCORE, GameState.WAITING_PLAYER_READY_BEFORE_GAME_RESTART ]:
            self.init_game()
            self.create_game()
